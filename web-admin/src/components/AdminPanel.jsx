...
const AdminPanel = () => {
  ...
  const loadPanelData = async () => {
    ...
    try {
      ...
      alert(`Unable to load admin dashboard: ${errMsg(err)}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '76vh', overflowY: 'auto', paddingRight: 6 }}>
      <div className='list-row' style={{ justifyContent: 'space-between', gap: 12 }}>
        <strong style={{ fontSize: '1.1rem' }}>Admin summary</strong>
        {loading && <span className='helper-text'>Updating…</span>}
      </div>
      <div className='panel' style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
        <div className='list-row' style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
          <span className='helper-text'>Orders</span>
          <strong>{orders.length}</strong>
        </div>
        <div className='list-row' style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
          <span className='helper-text'>Revenue</span>
          <strong>{formatCurrency(totalRevenue)}</strong>
        </div>
        <div className='list-row' style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
          <span className='helper-text'>Platform fees</span>
          <strong>{formatCurrency(totalPlatformFees)}</strong>
        </div>
        <div className='list-row' style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
          <span className='helper-text'>Open disputes</span>
        ...
        <div className='list' style={{ maxHeight: 210, overflowY: 'auto' }}>
          {orders.length ? orders.map((order) => (
            <div key={`admin-order-${order.id}`} className='list-row' style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
              <div>
                <strong>#{order.id}</strong> • {lookupUsername(order.buyer_id)} → {lookupUsername(order.seller_id)} • {order.listing_title || 'Listing'} • {formatCurrency(order.gross_amount, order.currency || 'GHS')} • {order.status || order.fulfillment_status || 'unknown'}
              </div>
              <button
                type='button'
        ...
        <div className='list' style={{ maxHeight: 210, overflowY: 'auto' }}>
          {disputes.length ? disputes.map((dispute) => (
            <div key={`admin-dispute-${dispute.id}`} className='list-row' style={{ flexDirection: 'column', gap: 6, borderBottom: '1px solid #e2e8f0', paddingBottom: 10 }}>
              <div><strong>Order #{dispute.order_id}</strong> • Buyer {lookupUsername(dispute.buyer_id)}</div>
          ...
          {completedPayouts.length ? completedPayouts.map((payout) => {
            ...
            return (
              <div key={`admin-payout-${payout.id}`} className='list-row' style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  {lookupUsername(payout.seller_id)} • {formatCurrency(payout.amount, payout.currency || 'GHS')} • Fee: {formatCurrency(order?.platform_fee_amount || order?.platform_fee || 0, payout.currency || 'GHS')} • {new Date(payout.created_at || payout.updated_at || payout.reference || Date.now()).toLocaleString()}
                </div>
              </div>
            )
