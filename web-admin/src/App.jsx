...
const [serviceCreateType, setServiceCreateType] = useState('logistics')
 ...
 const [logisticsForm, setLogisticsForm] = useState({ requester_id: 1, pickup_location: '', dropoff_location: '', cargo_type: '', weight_kg: '', status: 'PENDING', ships_from_country: 'GH', ships_from_city: '', ships_to_scope: 'country', shipping_cost_type: 'buyer_pays_actual', shipping_cost_amount: '', estimated_ship_days: '1-3 business days', shipping_notes: '' })
 const [logisticsEdit, setLogisticsEdit] = useState({ id: '', requester_id: 1, pickup_location: '', dropoff_location: '', cargo_type: '', weight_kg: '', status: 'PENDING', ships_from_country: 'GH', ships_from_city: '', ships_to_scope: 'country', shipping_cost_type: 'buyer_pays_actual', shipping_cost_amount: '', estimated_ship_days: '1-3 business days', shipping_notes: '' })
 const [equipmentForm, setEquipmentForm] = useState({ requester_id: 1, equipment_type: '', duration_days: '', location: '', budget: '', status: 'PENDING', service_delivery_mode: 'in_person', meeting_link: '', ships_from_country: 'GH', ships_from_city: '', ships_to_scope: 'country', shipping_cost_type: 'buyer_pays_actual', shipping_cost_amount: '', estimated_ship_days: '1-3 business days', shipping_notes: '' })
 const [equipmentEdit, setEquipmentEdit] = useState({ id: '', requester_id: 1, equipment_type: '', duration_days: '', location: '', budget: '', status: 'PENDING', service_delivery_mode: 'in_person', meeting_link: '', ships_from_country: 'GH', ships_from_city: '', ships_to_scope: 'country', shipping_cost_type: 'buyer_pays_actual', shipping_cost_amount: '', estimated_ship_days: '1-3 business days', shipping_notes: '' })
 const [storageForm, setStorageForm] = useState({ requester_id: 1, storage_type: '', quantity_kg: '', location: '', duration_days: '', status: 'PENDING', ships_from_country: 'GH', ships_from_city: '', ships_to_scope: 'country', shipping_cost_type: 'buyer_pays_actual', shipping_cost_amount: '', estimated_ship_days: '1-3 business days', shipping_notes: '' })
 ...
 <article className='panel'>
  ...
  {serviceCreateType === 'logistics' && <form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.createLogistics({ ...logisticsForm, ...normalizeListingImages(serviceImages), requester_id: Number(logisticsForm.requester_id || me?.id || 1), weight_kg: logisticsForm.weight_kg === '' ? 0 : Number(logisticsForm.weight_kg), ships_from_country: logisticsForm.ships_from_country || 'GH', ships_from_city: logisticsForm.ships_from_city || logisticsForm.pickup_location || 'Accra', ships_to_scope: logisticsForm.ships_to_scope || 'country', shipping_cost_type: logisticsForm.shipping_cost_type || 'buyer_pays_actual', shipping_cost_amount: logisticsForm.shipping_cost_amount === '' ? null : Number(logisticsForm.shipping_cost_amount), estimated_ship_days: logisticsForm.estimated_ship_days || '1-3 business days', shipping_notes: logisticsForm.shipping_notes || '' }); const created = res?.record || res; setState(prev => ({ ...prev, logistics: [created, ...(prev.logistics || [])] })); setServiceImages([]); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
  ...
  {serviceCreateType === 'storage' && <form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.createStorage({ ...storageForm, ...normalizeListingImages(serviceImages), requester_id: Number(storageForm.requester_id || me?.id || 1), duration_days: storageForm.duration_days === '' ? 1 : Number(storageForm.duration_days), quantity_kg: storageForm.quantity_kg === '' ? 0 : Number(storageForm.quantity_kg), ships_from_country: storageForm.ships_from_country || 'GH', ships_from_city: storageForm.ships_from_city || storageForm.location || 'Accra', ships_to_scope: storageForm.ships_to_scope || 'country', shipping_cost_type: storageForm.shipping_cost_type || 'buyer_pays_actual', shipping_cost_amount: storageForm.shipping_cost_amount === '' ? null : Number(storageForm.shipping_cost_amount), estimated_ship_days: storageForm.estimated_ship_days || '1-3 business days', shipping_notes: storageForm.shipping_notes || '' }); const created = res?.record || res; setState(prev => ({ ...prev, storage: [created, ...(prev.storage || [])] })); setServiceImages([]); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
  ...
  {serviceCreateType === 'consultation' && <form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.createEquipment({ ...equipmentForm, ...normalizeListingImages(serviceImages), requester_id: Number(equipmentForm.requester_id || me?.id || 1), duration_days: equipmentForm.duration_days === '' ? 1 : Number(equipmentForm.duration_days || 1), budget: equipmentForm.budget === '' ? 0 : Number(equipmentForm.budget), location: equipmentForm.service_delivery_mode === 'virtual' ? (equipmentForm.location || 'Virtual / Video Call') : equipmentForm.location, meeting_link: equipmentForm.meeting_link || undefined }); const created = res?.record || res; setState(prev => ({ ...prev, equipment: [created, ...(prev.equipment || [])] })); setServiceImages([]); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
   <select className='input' value={equipmentForm.equipment_type} onChange={e => setEquipmentForm({ ...equipmentForm, equipment_type: e.target.value })} required>
 ...
 <article className='panel'><h4>Edit Logistics</h4><form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.updateLogistics(Number(logisticsEdit.id), { ...logisticsEdit, ...normalizeListingImages(serviceEditImages), requester_id: Number(logisticsEdit.requester_id || me?.id || 1), weight_kg: logisticsEdit.weight_kg === '' ? 0 : Number(logisticsEdit.weight_kg) }); const updated = res?.record || res; setState(prev => ({ ...prev, logistics: (prev.logistics || []).map(row => Number(row.id) === Number(updated.id) ? { ...row, ...updated } : row) })); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
 ...
 <article className='panel'><h4>Edit Equipment</h4><form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.updateEquipment(Number(equipmentEdit.id), { ...equipmentEdit, ...normalizeListingImages(serviceEditImages), requester_id: Number(equipmentEdit.requester_id || me?.id || 1), duration_days: equipmentEdit.duration_days === '' ? 1 : Number(equipmentEdit.duration_days), budget: equipmentEdit.budget === '' ? 0 : Number(equipmentEdit.budget) }); const updated = res?.record || res; setState(prev => ({ ...prev, equipment: (prev.equipment || []).map(row => Number(row.id) === Number(updated.id) ? { ...row, ...updated } : row) })); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
 ...
 <article className='panel'><h4>Edit Storage</h4><form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.updateStorage(Number(storageEdit.id), { ...storageEdit, ...normalizeListingImages(serviceEditImages), requester_id: Number(storageEdit.requester_id || me?.id || 1), duration_days: storageEdit.duration_days === '' ? 1 : Number(storageEdit.duration_days), quantity_kg: storageEdit.quantity_kg === '' ? 0 : Number(storageEdit.quantity_kg) }); const updated = res?.record || res; setState(prev => ({ ...prev, storage: (prev.storage || []).map(row => Number(row.id) === Number(updated.id) ? { ...row, ...updated } : row) })); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
