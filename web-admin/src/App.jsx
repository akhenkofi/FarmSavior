...
const [serviceCreateType, setServiceCreateType] = useState('logistics')
 ...
 <article className='panel'><h4>Edit Logistics</h4><form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.updateLogistics(Number(logisticsEdit.id), { ...logisticsEdit, ...normalizeListingImages(serviceEditImages), requester_id: Number(logisticsEdit.requester_id || me?.id || 1), weight_kg: Number(logisticsEdit.weight_kg) }); const updated = res?.record || res; setState(prev => ({ ...prev, logistics: (prev.logistics || []).map(row => Number(row.id) === Number(updated.id) ? { ...row, ...updated } : row) })); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
 ...
 <article className='panel'><h4>Edit Equipment</h4><form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.updateEquipment(Number(equipmentEdit.id), { ...equipmentEdit, ...normalizeListingImages(serviceEditImages), requester_id: Number(equipmentEdit.requester_id || me?.id || 1), duration_days: Number(equipmentEdit.duration_days), budget: Number(equipmentEdit.budget) }); const updated = res?.record || res; setState(prev => ({ ...prev, equipment: (prev.equipment || []).map(row => Number(row.id) === Number(updated.id) ? { ...row, ...updated } : row) })); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
 ...
 <article className='panel'><h4>Edit Storage</h4><form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.updateStorage(Number(storageEdit.id), { ...storageEdit, ...normalizeListingImages(serviceEditImages), requester_id: Number(storageEdit.requester_id || me?.id || 1), duration_days: Number(storageEdit.duration_days), quantity_kg: Number(storageEdit.quantity_kg) }); const updated = res?.record || res; setState(prev => ({ ...prev, storage: (prev.storage || []).map(row => Number(row.id) === Number(updated.id) ? { ...row, ...updated } : row) })); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
