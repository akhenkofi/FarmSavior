...
const [serviceCreateType, setServiceCreateType] = useState('logistics')
 ...
 <article className='panel'>
  ...
  {serviceCreateType === 'equipment' && <form className='list' onSubmit={async e => { e.preventDefault(); try { const res = await api.createEquipment({ ...equipmentForm, ...normalizeListingImages(serviceImages), requester_id: Number(equipmentForm.requester_id || me?.id || 1), duration_days: Number(equipmentForm.duration_days), budget: Number(equipmentForm.budget) }); const created = res?.record || res; setState(prev => ({ ...prev, equipment: [created, ...(prev.equipment || [])] })); setServiceImages([]); setServicesView('list'); setActive('services'); } catch (err) { alert(errMsg(err)) } }}>
