...
function AppInner() {
 ...
 <div className='card-actions'>
 <button className={`btn ${effectiveLivestockSubscription?.tier === 'premium' ? '' : 'btn-dark'}`} onClick={() => { if (effectiveLivestockSubscription?.tier === 'premium') { if (token) window.location.href='/?public=0&go=livestock-records'; else setActive('livestock-records'); return } setActive('onboarding'); if (!token) setAuthOpen(true) }}>{effectiveLivestockSubscription?.tier === 'premium' ? 'Open records' : 'View upgrade plans'}</button>
 <button className='btn' onClick={() => { setActive('onboarding'); if (!token) setAuthOpen(true) }}>My billing</button>
 </div>
 </div>}
 </article>
