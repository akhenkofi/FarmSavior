...
def _require_transact_verified_user(db: Session, user_id: int, label: str = 'User'):
    ...
    approved = bool(getattr(user, 'is_verified', False)) or bool(getattr(user, 'identity_blue_check', False)) or (bool(review) and str(getattr(review, 'status', '')).upper() == 'APPROVED')
